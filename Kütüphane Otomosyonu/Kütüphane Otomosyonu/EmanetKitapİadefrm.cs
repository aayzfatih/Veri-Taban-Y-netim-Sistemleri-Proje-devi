...
namespace Kütüphane_Otomosyonu
{
    public partial class EmanetKitapİadefrm : Form
    {
        ...
        private void kimlikNoTextBox_TextChanged(object sender, EventArgs e)
        {
            daset.Tables["EmanetKitaplar"].Clear();
            baglanti.Open();
            SqlDataAdapter adtr = new SqlDataAdapter("select *from EmanetKitaplar where tc like'%" + kimlikNoTextBox.Text + "%'", baglanti);
            adtr.Fill(daset,"EmanetKitaplar");
            baglanti.Close();
            if (kimlikNoTextBox.Text==" ")
            {
                daset.Tables["EmanetKitaplar"].Clear();
                EmanetListele();
        ...
        {
            baglanti.Open();
            SqlDataAdapter adtr = new SqlDataAdapter("select *from EmanetKitaplar where barkodno like'%" + barkodNoTextBox.Text + "%'", baglanti);
            adtr.Fill(daset, "EmanetKitaplar");
            baglanti.Close();
        ...
        private void teslimAlButton_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Kitaplar iade edidi");
            baglanti.Close();
